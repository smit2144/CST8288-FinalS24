...
*/
@WebServlet(name = "Retailer_AddInventoryServlet", urlPatterns = {"/Retailer_AddInventoryServlet"})
public class Retailer_AddInventoryServlet extends HttpServlet {
            ...
            if ((surplusStatus.equals("yes") && (plan.equals("donation") || plan.equals("sale"))) ||(surplusStatus.equals("no"))){
                if ((plan.equals("donation") && price == 0.0) || (plan.equals("sale") && price != 0.0)||(plan.equals("keep") && price == 0.0)) {
                    ...
                    try {
                        ...
                        // Set inventory attribute in request for sending notifications
                        request.setAttribute("inventory", inventory);

                        // Set notification message attribute in request
                        request.setAttribute("notificationMessage", "Notification sent successfully");

                        // Forward request to both SendInventoryNotificationServlet and add inventory JSP page
                        RequestDispatcher dispatcher = request.getRequestDispatcher("/SendInventoryNotificationServlet");
